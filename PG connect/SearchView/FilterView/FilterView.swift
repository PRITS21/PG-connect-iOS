...
struct FilterView: View {
    ...
    @State private var selectedIndices_PG: Set<Int> = []
                        ...
                        ForEach(0..<PGType.count) { index in
                            Button(action: {
                                ...
                                Text(PGType[index])
                                    ...
                                    .font(.system(size: 11.5))
                                    .foregroundColor(selectedIndices_PG.contains(index) ? .white : .black)
                                    .padding(.horizontal, 8)
                                    .padding(.vertical, 8)
                                    .background(selectedIndices_PG.contains(index) ? Color(UIColor(hex: "#7F32CD")) : Color.white)
                                    .cornerRadius(15)
                                    .overlay(RoundedRectangle(cornerRadius: 15).stroke(lineWidth: 1).foregroundColor(.gray))
                            }}
                        Spacer()
    ...
    private func toggleSelection1(_ index: Int) {
        if selectedIndices_PG.contains(index) {
            selectedIndices_PG.remove(index)
        } else {
            selectedIndices_PG.insert(index)
        }
    }
